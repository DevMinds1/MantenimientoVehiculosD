...
export const CompletadoMantenimeinto = () => {
  ...
  useEffect(() => {
    ...
    setFallasSeleccionadas(ids); // Marca todas las casillas cuando el componente se monta
  }, [fallasConId]);
    ...
    setFallasSeleccionadas(
      (prev) =>
        prev.includes(fallaId)
          ? prev.filter((id) => id !== fallaId) // Deselect if already selected
          : [...prev, fallaId] // Select if not selected
    );
  };
  ...
  return (
    <ScrollView>
      <View style={globalStyles(top).container}>
        ...
        <View>
          <Text style={styles.label}>Valor a cancelar: $50</Text>
          <View style={{ justifyContent: "center", alignItems: "center" }}>
            <Text style={styles.textimg}>Factura</Text>
            <View style={styles.contanierimg}>
              <Image
                source={{
                  uri: "https://th.bing.com/th/id/OIP.DfPHoCovbdaaOz0G8XyIpAHaJl?rs=1&pid=ImgDetMain",
                }}
                style={{ width: "100%", height: "100%" }}
              />
            </View>
